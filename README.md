...
- 如何保证加载模块和消费模块的顺序
    - 因为整个ReactBoot核心都是在运行时执行，那么只需要确保模块加载的过程，在run方法(启动react应用)之前完成就行了
- 如何解决加载模块循环依赖的问题
    - 如果A组件中依赖了ReactBoot导出的函数(注解)，同时A组件又作为生产者，ReactBoot需要扫描注入A组件，那么两者就形成循环依赖，
      解决方法是使用Promise异步+生成器模式来扫描注入组件，将ReactBoot导出函数与扫描注入异步解耦，断开模块之间硬连接，避免阻塞，
      保证A模块在ReactBoot应用注册后、React应用启动前加载，此时ReactBoot导出了函数，但由于模块扫描yield异步暂停了，所以函数具体实现没有完成，也就是
      A组件可以顺利引入函数，此时的函数更像一个声明，没有具体实现，待A等组件同步引入函数代码执行完成后，异步的模块扫描next开始执行，最后在执行完成后
      的then回调中启动React应用,此时模块已经扫描注入完成，函数有了具体实现，此时应用的运行时组件就能正常消费函数了
- IOC容器采用什么方式来实现的
